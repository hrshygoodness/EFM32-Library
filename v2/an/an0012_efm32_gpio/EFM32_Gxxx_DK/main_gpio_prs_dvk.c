...
{  
  ...
  /* Configure PE1 as an input with filter enable (out = 1)*/
  GPIO_PinModeSet(gpioPortE, 1, gpioModeInputPullFilter, 1); 
  
  /* Select PE1 as PRS output */
  GPIO->EXTIPSELL = (GPIO->EXTIPSELL & !_GPIO_EXTIPSELL_EXTIPSEL1_MASK ) | GPIO_EXTIPSELL_EXTIPSEL1_PORTE;
  
  /* Enable PRS sense */
  GPIO->INSENSE = GPIO->INSENSE | GPIO_INSENSE_PRS;
  
  /* Select GPIO as source and GPIOPIN1 as signal (falling edge) */
  PRS_SourceSignalSet(0, PRS_CH_CTRL_SOURCESEL_GPIOL, PRS_CH_CTRL_SIGSEL_GPIOPIN1, prsEdgeNeg);
