...
int main(void)
{
  ...
  xTaskCreate(DrawPicture, (const signed char *) "DrawPicture", STACK_SIZE_FOR_TASK, NULL, TASK_PRIORITY, NULL);
  /* Create task to toggle the polarity inversion of the Sharp memory LCD.  */
  xTaskCreate(RepeatCallbackTask, (const signed char *) "RepeatToggle", STACK_SIZE_FOR_TASK, NULL, DELAY_PRIORITY, NULL);
